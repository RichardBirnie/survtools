...
#'
#' @export
ggsurvplot2 <- function(km, cen = FALSE, psm.curves, xval, yval, groups, plot.title = '', time.interval = 1,
                        show.table = TRUE, table.height = 0.2, table.title = 'Number at risk',
                        title.legend = '', labs.legend, l.type, x.limit, x.label = 'Time (Years)',
                        cols = 'hue', ...) {
  fig <- survminer::ggsurvplot(km, break.time.by = time.interval, palette = cols, xlab = x.label,
      risk.table = show.table, risk.table.y.text.col = FALSE, risk.table.height = table.height,
      risk.table.title = table.title, legend.title = title.legend, legend.labs = labs.legend,
      censor = cen, linetype = l.type, xlim = x.limit, ...
    )
  ...
  fig$plot <- fig$plot + ggplot2::geom_line(data = psm.curves[[i]],
    ...
    # scale_linetype_manual(values=c("solid","dashed"),na.value="dashed")+
